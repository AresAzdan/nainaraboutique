...
const handleNotification = async (req, res, next) => {
  try {
    ...
    // 2. Parse our internal order ID from "ORDER-{id}-{timestamp}"
    ...
    const orderId = parts[1]; // numeric database ID

    // 3. Resolve target status
    const targetStatus = resolveOrderStatus(transactionStatus, fraudStatus);

    if (targetStatus === null) {
      console.log(`[Webhook] No status change for order ${orderId} (${transactionStatus})`);
      return res.status(200).json({ message: 'No status change required.' });
    }

    // 4. Fetch current order to guard invalid transitions
    const order = await OrderModel.findById(orderId);
    if (!order) {
      console.warn(`[Webhook] Order ${orderId} not found in DB for notification ${rawOrderId}`);
  ...
  console.error('[Webhook] Unexpected error FULL:', err); // already there
